...
#
#Filter output by population from dictionary

#Define main function-----------------------------------------------------------------------------------------------------
def main():
     pops=make_pop_dict() #creates population dictionary
     filter_dw_data(pops) #filters drinking water data from population dictionary
...
def filter_dw_data(pops):
     ...
     for line in dw_infile: #for loop that reorganizes file lines in data file
          ...
          if pop_dict.get(country,0)>=500000: #read dictionary and find (get) countries with population>500,000
               print(country, int(column[1]), int(column[21]), percent_change) #only print countries that satisfy condition
